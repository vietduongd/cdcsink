...
impl ConfigStore {
    ...
    pub fn load(storage_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        let connectors = if connectors_path.exists() {
            ...
            let list: Vec<ConnectorConfigEntry> =
                serde_yaml::from_str(&content).context("Failed to parse connectors.yaml")?;
        ...
        let destinations = if destinations_path.exists() {
            ...
            let list: Vec<DestinationConfigEntry> =
                serde_yaml::from_str(&content).context("Failed to parse destinations.yaml")?;
            list.into_iter().map(|e| (e.name.clone(), e)).collect()
        } else {
            HashMap::new()
        };
        ...
        let flows = if flows_path.exists() {
            let content =
                std::fs::read_to_string(&flows_path).context("Failed to read flows.yaml")?;
            let list: Vec<FlowConfigEntry> =
                serde_yaml::from_str(&content).context("Failed to parse flows.yaml")?;
            list.into_iter().map(|e| (e.name.clone(), e)).collect()
        } else {
            HashMap::new()
        };
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let connectors_yaml =
            serde_yaml::to_string(&connectors_list).context("Failed to serialize connectors")?;
        ...
        std::fs::write(
            self.storage_dir.join("destinations.yaml"),
            destinations_yaml,
        )
        ...
        let flows_yaml = serde_yaml::to_string(&flows_list).context("Failed to serialize flows")?;
    ...
    pub async fn delete_connector(&mut self, name: &str) -> Result<()> {
        ...
        // Get connector info before deletion for cleanup
        let connector = self
            .connectors
            .get(name)
            .ok_or_else(|| anyhow!("Connector '{}' not found", name))?;

        // Attempt to cleanup external resources
        if let Err(e) =
            Self::cleanup_connector_resources(&connector.connector_type, &connector.config).await
        {
            tracing::warn!("Cleanup failed for connector '{}': {}", name, e);
    ...
    pub fn update_destination(
        &mut self,
        name: &str,
        mut entry: DestinationConfigEntry,
    ) -> Result<()> {
    ...
    /// Clean up external resources for a connector before deletion
    /// This is a best-effort operation - failures are logged but don't prevent deletion
    async fn cleanup_connector_resources(
        connector_type: &str,
        ...
        match connector_type {
            "nats" => {
                // Parse NATS config
                let nats_config: cdc_nats_connector::NatsConfig =
                    serde_json::from_value(config.clone())
                        .context("Failed to parse NATS config")?;

                // Call cleanup directly with the config
                info!("Cleaning up NATS connector resources");
                match cdc_nats_connector::cleanup_nats_consumer(&nats_config).await {
                    Ok(_) => {
                        info!("Successfully cleaned up NATS connector resources");
                        Ok(())
                    }
                    Err(e) => {
                        warn!("Failed to cleanup NATS connector resources: {}", e);
                        // Don't fail deletion on cleanup errors
                        Ok(())
                    }
                }
            }
            // Add other connector types here as they implement cleanup
            // "kafka" => { ... }
            _ => {
                info!(
                    "Connector type '{}' does not require cleanup",
                    connector_type
                );
                Ok(())
            }
...
mod tests {
    ...
    async fn test_config_store_crud() {
        ...
        assert!(store.delete_connector("test-nats").await.is_err());

        // Delete flow first
        store.delete_flow("test-flow").unwrap();

        // Now can delete connector
        store.delete_connector("test-nats").await.unwrap();
